...
class Environment:
    ...
    def update_greedy_path(self,greedy_path):
        self.greedy_path = greedy_path

    def draw_greedy_path(self):
        for pair in self.greedy_path:
            state1,state2=pair
            # print(state1,state2)
            # exit()
            x1 = int(state1[0]*self.magnification)
            y1 = int((1-state1[1])*self.magnification)
            x2 = int(state2[0]*self.magnification)
            y2 = int((1-state2[1])*self.magnification)
            cv2.line(self.image,(x1,y1),(x2,y2),(0,255,0),thickness=2)
    ...
    def save(self):
        cv2.imwrite('maze.png', self.image)
